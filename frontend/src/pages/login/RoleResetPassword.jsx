...
function RoleResetPassword({ role, loginPath, accountPurpose }) {
  ...
  const handleSubmit = (event) => {
    ...
    else if (!passwordChecks.every((check) => check.test(form.password))) nextErrors.password = 'Password does not meet the requirements.';

    if (!form.confirmPassword.trim()) nextErrors.confirmPassword = 'Please confirm your new password.';
  ...
  return (
    <LoginShell
      pageTitle={`${role} Reset Password`}
      pageDescription="Set a new password."
      notice="Choose a strong password to keep your account safe."
      leftTitle="Set a new password"
      leftDescription={`Create a strong, unique password for your ${accountPurpose} so it stays protected and ready for your next ${role === 'Customer' ? 'booking' : 'order'}.`}
    ...
    >
      {isSuccess ? (
        ...
        <form className="space-y-6" onSubmit={handleSubmit} noValidate>
          <PasswordField label="New Password" name="password" value={form.password} onChange={updateField} error={errors.password} placeholder="Enter a strong password" showPassword={showPassword} onToggleShowPassword={() => setShowPassword((value) => !value)} />
          <PasswordField label="Confirm New Password" name="confirmPassword" value={form.confirmPassword} onChange={updateField} error={errors.confirmPassword} placeholder="Re-enter your password" showPassword={showConfirmPassword} onToggleShowPassword={() => setShowConfirmPassword((value) => !value)} />
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700">
            ...
            <ul className="mt-3 space-y-2">
              {passwordChecks.map((check) => {
                ...
                return <li key={check.label} className={`flex items-center gap-2 ${passed ? 'text-emerald-600' : 'text-slate-600'}`}><span>{passed ? 'âœ“' : 'â€¢'}</span>{check.label}</li>;
              })}
            </ul>
          </div>
...
export default RoleResetPassword;
