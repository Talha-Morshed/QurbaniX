    ...
    <aside className="butcher-sidebar">
      ...
      <nav aria-label="Butcher dashboard navigation">
        {navigation.map(([label, icon]) => {
          const path = label === 'Dashboard' ? '/dashboard/butcher' : label === 'Bookings' ? '/dashboard/butcher/bookings' : label === 'Services & Pricing' ? '/dashboard/butcher/services' : label === 'Availability' ? '/dashboard/butcher/availability' : label === 'Earnings' ? '/dashboard/butcher/earnings' : `#${label.toLowerCase().replaceAll(' ', '-')}`;
          const isActive = location.pathname === path;
          const item = <><Icon name={icon} /><span>{label}</span></>;

          return ['Dashboard', 'Bookings', 'Services & Pricing', 'Availability', 'Earnings'].includes(label) ? <Link key={label} to={path} className={`butcher-nav-item ${isActive ? 'is-active' : ''}`}>{item}</Link> : <a key={label} href={path} className={`butcher-nav-item ${isActive ? 'is-active' : ''}`}>{item}</a>;
        })}
      </nav>
