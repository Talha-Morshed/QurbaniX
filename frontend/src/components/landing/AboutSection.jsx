    ...
    <section id="about" className="bg-gray-50 border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16 lg:py-24">
        <div className="max-w-2xl mx-auto text-center">
          <span className="inline-flex items-center px-3 py-1 bg-emerald-100 text-emerald-700 font-semibold text-xs sm:text-sm tracking-widest">About QurbaniX</span>
          <h2 className="mt-4 sm:mt-6 text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight text-emerald-900">Qurbani, Done The Right Way</h2>
        ...
        <div className="mt-8 sm:mt-12 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          {features.map((feature) => (
            <div
              key={feature.title}
              className={`premium-action group p-5 sm:p-6 md:p-8 flex flex-col items-start gap-4 sm:gap-5 shadow-sm hover:shadow-xl ${
                feature.highlight ? 'bg-emerald-900 ring-1 ring-emerald-900' : 'bg-white ring-1 ring-slate-200'
              }`}
            >
              <div className={`inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 ${feature.highlight ? 'bg-emerald-800 text-white' : 'bg-emerald-900 text-[#D4A72C]'}`}>
                {feature.icon}
              </div>
              <h3 className={`text-lg sm:text-xl font-bold ${feature.highlight ? 'text-white' : 'text-emerald-900'}`}>{feature.title}</h3>
              <p className={`text-sm sm:text-base leading-relaxed ${feature.highlight ? 'text-white' : 'text-slate-600'}`}>{feature.desc}</p>
            </div>
          ))}
        </div>

        <div className="mt-8 sm:mt-12 bg-emerald-900 px-5 sm:px-8 py-6 sm:py-8 lg:px-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 sm:gap-6">
          <p className="text-base sm:text-lg lg:text-xl font-medium text-emerald-50 max-w-3xl">
